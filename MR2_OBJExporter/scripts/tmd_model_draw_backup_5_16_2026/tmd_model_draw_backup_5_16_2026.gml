...
function tmd_model_draw_backup_5_16_2026(_prim_array, _object, _x, _y, _view1 = 0, _view2 = 0, _offset, _count, _fill, _line, _vertex_color = 0, _scale = 1, _transparency = 1){
	prim_array = [];
	vert_size = 2;
	xsedit = 1;
	ysedit = 1;
	...
	for (var i = 0; i < _count; i++){
		...
		c_list = [c_black, c_black, c_black, c_black];
		...
		draw_vertex_colour(x_loc + x_list[0], y_loc + y_list[0], c_list[0], 1);
		draw_vertex_colour(x_loc + x_list[1], y_loc + y_list[1], c_list[1], 1);
		draw_vertex_colour(x_loc + x_list[2], y_loc + y_list[2], c_list[2], 1);
			
		if (prim_array[i + _offset].cmd & 0b1000 == 8){	//fourth vertex check
			draw_vertex_colour(x_loc + x_list[1], y_loc + y_list[1], c_list[1], 1);
			draw_vertex_colour(x_loc + x_list[2], y_loc + y_list[2], c_list[2], 1);
			draw_vertex_colour(x_loc + x_list[3], y_loc + y_list[3], c_list[3], 1);
		}
		draw_primitive_end();
