...
function tmd_vertex_draw_backup_5_16_2026(_prim_array, _offset, _count, _fill, _line, _edit = 0, _vertex_color = 0, _template = 0, _transparency = 1, _ignore_page = 0){
	prim_array = [];
	xedit = 0;
	yedit = 0;
	vert_size = 2;
	...
	//	prim_rotate = rotate + prim_array[i + _offset].r_total;
		...
		if (ui_page - 16 == page || ignore_page == 1){
			...
			//	//draw_set_colour(_fill);
			//draw_set_alpha(_transparency * .2);
			for (var b = 0; b < array_length(prim_array[i + _offset].colors); b++){
				...
				draw_vertex_colour(2 + grid_x + x_list[0], 2 + grid_y + y_list[0], c_list[0], transparency);
				draw_vertex_colour(2 + grid_x + x_list[1], 2 + grid_y + y_list[1], c_list[1], transparency);
				draw_vertex_colour(2 + grid_x + x_list[2], 2 + grid_y + y_list[2], c_list[2], transparency);
			
				if (prim_array[i + _offset].cmd & 0b1000 == 8){	//fourth vertex check
					draw_vertex_colour(2 + grid_x + x_list[1], 2 + grid_y + y_list[1], c_list[1], transparency);
					draw_vertex_colour(2 + grid_x + x_list[2], 2 + grid_y + y_list[2], c_list[2], transparency);
					draw_vertex_colour(2 + grid_x + x_list[3], 2 + grid_y + y_list[3], c_list[3], transparency);
				}
			...
			draw_vertex(2 + grid_x + (x_list[0] * 1), 2 + grid_y + (y_list[0]* 1));
			draw_vertex(2 + grid_x + (x_list[1] * 1), 2 + grid_y + (y_list[1]* 1));
			draw_vertex(2 + grid_x + (x_list[2] * 1), 2 + grid_y + (y_list[2]* 1));
			draw_vertex(2 + grid_x + (x_list[0] * 1), 2 + grid_y + (y_list[0]* 1));
			draw_primitive_end();
			...
			if (prim_array[i + _offset].cmd & 0b1000 == 8){	//fourth vertex check
				draw_primitive_begin(pr_linestrip);
				draw_vertex(2 + grid_x + (x_list[1] * 1), 2 + grid_y + (y_list[1]* 1));
				draw_vertex(2 + grid_x + (x_list[3] * 1), 2 + grid_y + (y_list[3]* 1));
				draw_vertex(2 + grid_x + (x_list[2] * 1), 2 + grid_y + (y_list[2]* 1));
				draw_primitive_end();
			}
